...
import { getWrongStats, getWrongSummary, useStudy } from "@/lib/store";

type WrongCard = QuizCard & {
  wrongCount: number;
  correctCount: number;
  totalCount: number;
...
/**
 * 错题本：从 localStorage 的历史作答明细里聚合出答错过的题，
 * 按错误次数排序并加权重练。数据全部来自本机，无需服务器。
 */
export function ReviewBoard({ bank }: { bank: QuizCard[] }) {
  const state = useStudy();
  const summary = getWrongSummary(state);
  const byId = new Map(bank.map((question) => [question.id, question]));

  const wrong: WrongCard[] = getWrongStats(state).flatMap((stat) => {
    ...
    <div className="space-y-9">
      <header>
        ...
        <p className="mt-3 max-w-3xl text-[14.5px] leading-7 text-slate-400">
          ...
          错得越多的题出现概率越高（加权抽题），用来把薄弱点练熟。数据来自本机浏览器保存的作答明细，无需额外记录。
        </p>
      ...
      {wrong.length === 0 ? (
        ...
        <>
          ...
          <section>
            ...
            <div className="mt-5">
              <QuizRunner bank={wrong} docOptions={[]} weights={weights} recordSlug="wrong" lockScope />
            </div>
          </section>
        </>
      )}
